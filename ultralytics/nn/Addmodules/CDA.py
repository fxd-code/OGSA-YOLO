...
class Attention_Advanced(nn.Module):
    """方向感知的注意力生成器 (Direction-Aware Attention Generator) 创新点一：通过全局、水平、垂直三个池化分支来增强对方向和边缘的感知能力 解决"左偏移"问题：能够精准感知皮带边缘的方向性特征.
    """
    ...
    def forward(self, x):
        """前向传播：融合全局、水平、垂直三个池化分支的信息 形成"超级特征向量"来指导注意力的生成.
        """
        _B, _C, _H, _W = x.size()
        ...
        return (
            self.func_channel(x_fused),
            self.func_filter(x_fused),
            self.func_spatial(x_fused),
            self.func_kernel(x_fused),
        )


class MultiScaleContext(nn.Module):
    """输入端上下文增强模块 (Input-end Context Enhancement Module) 创新点二：使用多尺度空洞卷积来扩大感受野，解决"大块煤"问题 让模型在处理局部特征时能够感知到更大范围的上下文信息.

    改进版（方案一）：保留原始空间信息 在融合阶段将原始输入x也拼接进来，确保最原始、最精确的空间位置信息 不会在空洞卷积过程中被稀释，从而缓解定位精度下降的问题
    """
    ...
    def __init__(self, in_planes, dilation_rates=[1, 2, 3]):
        ...
        for dilation in dilation_rates:
            ...
            self.dilated_convs.append(
                nn.Conv2d(in_planes, in_planes, kernel_size=3, padding=padding, dilation=dilation, bias=False)
            )
    ...
    def forward(self, x):
        """前向传播：通过多个空洞卷积分支提取多尺度上下文，然后融合 改进：将原始输入x也拼接进来，形成一个包含"高精度位置信息"的超级特征图.
        """
        # 通过各个空洞卷积分支
        branch_outputs = []
...
class ODConv2d_Advanced(nn.Module):
    """改进版ODConv，集成两大创新点： 1. 方向感知的注意力生成器（Attention_Advanced）- 解决"左偏移"问题 2. 输入端上下文增强模块（MultiScaleContext）- 解决"大块煤"问题.
    """
    ...
    def __init__(
        ...
        # 创新点一：方向感知的注意力生成器
        self.attention = Attention_Advanced(
            in_planes, out_planes, kernel_size, groups=groups, reduction=reduction, kernel_num=kernel_num
        )
...
class Bottleneck_ODConv_Advanced(nn.Module):
    """使用ODConv2d_Advanced的瓶颈层 在动态卷积之后添加BatchNorm和激活函数来稳定训练.
    """
...
class Bottleneck_Hybrid_AttentiveFusion(nn.Module):
    ...
    改进版（方案二）：使用注意力机制进行智能融合 让 Bottleneck_Hybrid 中的两个分支（ODConv 和 Context）进行更智能的"软融合"，
    而不是简单的拼接。引入轻量级的通道注意力机制（SE-Block风格），让网络根据 输入特征，动态地学习给 odconv_branch 和 context_branch 的输出分配不同的权重。
...
class C3k2_HybridFusion(C2f):
    """一个遵循 C3k2 命名和结构风格的模块， 但其核心处理单元是我们最新设计的、性能更优的 Bottleneck_Hybrid_AttentiveFusion。.

    这个模块可以直接在 YAML 文件中替换 C3k2_ODConv_Advanced， 是进行最终对比实验的推荐模块。
    """
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            ...
            if c3k
            else Bottleneck_Hybrid_AttentiveFusion(self.c, self.c, shortcut, g, e=1.0)
            for _ in range(n)
        )
...
class C3k2_ODConv_Advanced(C2f):
    """改进版C3k2模块，使用ODConv2d_Advanced 专门针对皮带机运行状态检测任务优化： - 解决"左偏移"问题：通过方向感知注意力增强边缘检测 - 解决"大块煤"问题：通过多尺度上下文增强扩大感受野.
    """
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        # 使用改进版的Bottleneck
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck_ODConv_Advanced(self.c, self.c, shortcut, g)
            for _ in range(n)
        )
