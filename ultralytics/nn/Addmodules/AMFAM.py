...
__all__ = ("Add", "CategoryAwareDynamicASF", "Dy_Sample", "DynamicScalSeq", "ScalSeq", "Zoom_cat", "attention_model")
...
class CategoryAwareDynamicASF(nn.Module):
    """动态类别感知ASF模块 (Category-Aware Dynamic ASF) 实现三大核心改进机制： 1. 类别引导的动态尺度权重生成 2. 尺度间语义一致性约束 3. 类别专属的双模式注意力增强 4.
    使用Dy_Sample进行自适应上采样.
    """
    ...
    def forward(self, x, class_emb=None, conf_maps=None):
        ...
        if conf_maps is not None:
            ...
            # 在冲突区域以高置信度尺度为主
            p4_fused = torch.where(conflict_region_p4, p3_weighted * 0.1 + p4_weighted * 0.9, p3_weighted + p4_weighted)
            p5_fused = torch.where(conflict_region_p5, p3_weighted * 0.1 + p5_weighted * 0.9, p3_weighted + p5_weighted)
