...
# 定义模块的对外接口
__all__ = ['SCSAB', 'C2PSASCSAB', 'Conv', 'PSABlock'] 

class SCSAB(BaseModule):
    """
    FULLY UPGRADED SCSAB Module (Directional 2D Enhancement + Hybrid Channel Attention).
    This version includes:
    1. Direction-aware strip convolutions for enhanced edge detection and directional feature perception (IMPROVEMENT 1).
    2. Hybrid channel attention (Self-Attention + Convolutional Attention) with adaptive fusion
       and refined self-attention channel weight generation for robust large object and offset detection (IMPROVEMENT 2).
    """
    def __init__(
            self,
            dim: int,
            ...
            group_kernel_sizes: t.List[int] = [3, 5, 7, 9], # Kernel sizes for SMSA's 1D DWCs
            qkv_bias: bool = False,
            fuse_bn: bool = False, # Unused in this version
            norm_cfg: t.Dict = dict(type='BN'), # Unused directly, GroupNorm is used
            act_cfg: t.Dict = dict(type='ReLU'), # Unused directly, Sigmoid/Softmax is used
        ...
        self.strip_conv_h = nn.Conv2d(dim, dim, kernel_size=(1, strip_kernel_size), padding=(0, strip_kernel_size // 2), groups=dim, bias=False)
        self.strip_conv_w = nn.Conv2d(dim, dim, kernel_size=(strip_kernel_size, 1), padding=(strip_kernel_size // 2, 0), groups=dim, bias=False)
        ...
        self.final_ca_gate = nn.Softmax(dim=1) if gate_layer == 'softmax' else nn.Sigmoid() # Final activation for combined weights
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        attn_sa_rearranged = rearrange(attn_sa_raw, 'b head_num head_dim (h w) -> b (head_num head_dim) h w', h=h_s, w=w_s)
        ...
        if h_s != h_orig or w_s != w_orig: 
            attn_sa_full_res = F.interpolate(attn_sa_rearranged, size=(h_orig, w_orig), mode='bilinear', align_corners=False)
        ...
        attn_conv_weights = self.conv_ca(y_conv).transpose(-1, -2).unsqueeze(-1) # Apply 1D conv, reshape to (B, C, 1, 1)
